...
export default function Analysis() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (saved) {
        ...
        setTickCount(
          savedCount ? parseInt(savedCount) : 0
        )
      }
    } catch {
      // ignore
  ...
  const getPercentages = (source: number[]) => {
    ...
    return Array.from({ length: 10 }, (_, i) => {
      ...
      return parseFloat(
        ((count / source.length) * 100).toFixed(1)
      )
    })
  }
  ...
  const getOverUnder = () => {
    ...
    const overCount = visibleDigits.filter(
      d => d >= 5
    ).length

    const underCount =
      visibleDigits.length - overCount

    return {
      over: parseFloat(
        ((overCount / visibleDigits.length) * 100).toFixed(1)
      ),
      under: parseFloat(
        ((underCount / visibleDigits.length) * 100).toFixed(1)
      ),
    }
  }
    ...
    >
      ...
      >
        ...
        <div className="ou-bar-track">
          <div
            className="ou-bar-segment"
            style={{
              width: `${overUnder.under}%`,
              background: '#ef4444',
            }}
          >
            {overUnder.under > 8
      ...
      >
        ...
        <div className="digit-grid">
          {Array.from({ length: 10 }, (_, i) => {
            ...
            const isHot =
              hasEnoughData && pct === maxPct

            const isCold =
              hasEnoughData && pct === minPct
