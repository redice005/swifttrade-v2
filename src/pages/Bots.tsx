...
export default function Bots() {
  ...
  // Bot state refs (to avoid stale closures)
    ...
    const unsub = subscribe((data) => {
      if (data.msg_type === 'balance') {
        ...
        if (state.running && state.startingBalance > 0) {
          ...
          const diff = currentBal - state.startingBalance
      ...
      if (data.msg_type === 'proposal_open_contract') {
        ...
        const profit = won ? contract.profit : -contract.buy_price
        ...
        <div style={{ background: '#1a1a2e', borderRadius: '12px', padding: '1.5rem' }}>
          ...
          {tradeLogs.map(log => (
            <div key={log.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '0.5rem 0', borderBottom: '1px solid #0a0a1a' }}>
              <span style={{ color: '#fff' }}>{log.type} · ${log.stake.toFixed(2)}</span>
              <span style={{ color: log.result === 'won' ? '#22c55e' : log.result === 'lost' ? '#ef4444' : '#aaa', fontWeight: 'bold' }}>
